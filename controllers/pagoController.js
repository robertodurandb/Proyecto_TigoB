class PagoController{
    ...
    static todolist (req, res){
        req.getConnection((err, conn)=>{
            ...
            conn.query('SELECT dc.cliente_dnicliente, dc.num_contrato, cl.apellidocli, cl.nombrecli, pl.nombreplan, pl.precioplan, pg.idpago, date_format(pg.fechapago, "%d-%m-%Y") as fechapago, pg.montopago, pg.mediopago, pg.mespago, pg.anio FROM pago as pg INNER JOIN detallecontrato as dc on dc.num_contrato = pg.num_contrato INNER JOIN cliente as cl on cl.dnicliente = dc.cliente_dnicliente INNER JOIN planes as pl on pl.idplanes = dc.planes_idplanes', (err, rows)=>{
                if(err) return res.send(err)
