class DetalleController {
    ...
    static list (req, res){
        req.getConnection((err, conn)=>{
            ...
            conn.query('select dc.num_contrato, dc.diapago, dc.planes_idplanes, pl.nombreplan, dc.cliente_dnicliente, date_format(dc.fecha_contrato, "%Y-%m-%d") as fecha_contrato, date_format(dc.fecha_instalacion, "%Y-%m-%d") as fecha_instalacion, dc.observacion, dc.instalacion from detallecontrato as dc INNER JOIN planes as pl on dc.planes_idplanes=pl.idplanes', (err, rows)=>{
                if(err) return res.send(err)
