export class DetalleController {
    ...
    static todoinstacli (req, res){
        req.getConnection((err, conn)=>{
            ...
            conn.query('select dc.num_contrato, dc.diapago, dc.observacion, date_format(dc.fecha_contrato, "%d-%m-%Y") as fecha_contrato, date_format(dc.fechaprog_instalacion, "%d-%m-%Y") as fechaprog_instalacion, dc.estadodc_instalacion, dc.instalacion_idinstalacion, cl.dnicliente, cl.nombrecli, cl.apellidocli, cl.distritocli, cl.direccioncli, cl.telefonocli, cl.fecha_nacimiento, pl.nombreplan, pl.precioplan, pl.velocidadplan, date_format(it.fechainstalacion, "%d-%m-%Y") as fechainstalacion, it.geolocalizacion, it.user_create, it.estado, it.observacion_instalacion, im.idimagen, im.nombreimg from detallecontrato as dc INNER JOIN cliente as cl on dc.cliente_dnicliente=cl.dnicliente INNER JOIN planes as pl on dc.planes_idplanes=pl.idplanes INNER JOIN instalacion as it on dc.instalacion_idinstalacion=it.idinstalacion INNER JOIN imagen as im on it.imagen_idimagen=im.idimagen', (err, rows)=>{
                if(err) return res.send(err)
